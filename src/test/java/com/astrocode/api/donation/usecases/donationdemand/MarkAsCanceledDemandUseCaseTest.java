...
class MarkAsCanceledDemandUseCaseTest {

    @Mock
    private DonationDemandRepository donationDemandRepository;

    @InjectMocks
    private MarkAsCanceledDemandUseCase useCase;
    ...
    void setUp() {
        ...
        instantUtilsMock.when(InstantUtils::now).thenReturn(Instant.parse("2026-03-01T14:00:00Z"));
    }
    ...
    @Test
    @DisplayName("Deve cancelar a demanda e salvar")
    void shouldCancelAndSave() {
        UUID demandId = UUID.randomUUID();
        MarkAsCanceledDemandCommand command = new MarkAsCanceledDemandCommand(demandId);

        DonationDemand activeDemand = DonationDemand.create(
                UUID.randomUUID(),
        ...
        when(donationDemandRepository.findById(demandId)).thenReturn(Optional.of(activeDemand));
        ...
        assertEquals(DemandStatus.CANCELED, result.getDemandStatus());
    ...
    void shouldThrowExceptionWhenNotFound() {
        MarkAsCanceledDemandCommand command = new MarkAsCanceledDemandCommand(UUID.randomUUID());
